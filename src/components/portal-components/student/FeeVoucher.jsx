          ...
          <div>
            <div class="rounded-lg border border-black bg-white p-2 shadow-lg">
              ...
              <div class="flex justify-between">
                ...
                <div class="ml-4">
                  <img src="${photoUrl || '/profilelogo.jpg'}" class="object-contain" width="100" height="100" onerror="this.src='/profile.jpg'">
                </div>
              </div>
              <!-- Bank Information -->
              ...
              <div class="grid grid-cols-2">
                <div class="py-1">Std Id: <strong>${
                  student.registrationNo || ""
                }</strong></div>
                <div class="py-1">Campus: <strong>${
                  student.campusName || ""
                }</strong></div>
              </div>
              <!-- Student Info -->
              <div class="flex flex-col">
                <div class="py-1">Name: <strong>${
                  student.name || ""
                }</strong></div>
                <div class="py-1">F.Name: <strong>${
                  student.fatherName || ""
                }</strong></div>
                <div class="grid grid-cols-2"> 
                  <div class="py-1">Class: <strong>${
                    student.className || ""
                  }</strong></div>
              ...
              <div class="my-1 space-y-2">
                <h2 class="text-center text-sm font-bold">Fee Voucher For The Month Of ${
                  feeRecords.at(-1)?.month
                }</h2>
                <div class="flex flex-row justify-between border-b border-black border-t py-0.5 text-sm">
                  <div class="flex-1 text-left">Issue Date: <strong>${new Date(
                    `${feeRecords.at(-1)?.month} 1, ${feeRecords.at(-1)?.year}`
                  ).toLocaleDateString()}</strong></div> 
                  <div class="flex-1 text-right">Due Date:<strong> ${new Date(
                    `${feeRecords.at(-1)?.month} 10, ${feeRecords.at(-1)?.year}`
                  ).toLocaleDateString()}</strong></div>
                </div>
                    ...
                    ${
                      arrears - feeRecords.at(-1).totalFee > 0
                        ? `<tr>
                            ...
                            <td class="border border-black px-1 py-0.5 text-center">${
                              arrears - feeRecords.at(-1).totalFee
                            }</td>
                          </tr>`
                        : ""
                    }
                ...
                <div class="mt-1 text-center">
                  <p><strong>Total Amount:</strong> ${
                    arrears + 10 + otherTotal || 0
                  }</p>
                  <p><strong>Amount After Due Date:</strong>${
                    arrears + 10 + 100 + otherTotal
                  }</p>
                </div>
              </div>
                    ...
                    .map(
                      (record, idx) => `
                        <tr>
                          ...
                          <td class="border border-black px-1 py-0.5 text-center">${
                            record.paidFeeAmount || record.status || "Not Paid"
                          }</td>
                        </tr>
                      `
                    )
           ...
           <div class="mt-5 space-x-6 text-xxs flex justify-between">
